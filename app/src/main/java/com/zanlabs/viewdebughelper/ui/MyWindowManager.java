	...
	public static void createFloatWindow(Context context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (!Settings.canDrawOverlays(context)) {
                Toast.makeText(context, "Please permit drawing over other apps permission on to let software work!", Toast.LENGTH_LONG).show();
		...
		if (mFloatWindow != null) {
			...
			if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.LOLLIPOP){
				if(AccessibilityServiceHelper.isAccessibilitySettingsOn(context, ViewDebugHelperApplication.getInstance().getCurrentPackName()+"/"+ ViewDebugHelperService.class.getName())){
                    sb.append(ViewDebugHelperApplication.getInstance().getLastTopActivityName());
                    sb.append("\nFragments:");
                    sb.append(ViewDebugHelperApplication.getInstance().getCurrentAllFragmentNames());
                }else{
                    sb.append("服务未开，数据不一定准确\n").append(ActivityHelper.getTopActivity(context));
				}
			}else{
				sb.append(ActivityHelper.getTopActivity(context));
			}
